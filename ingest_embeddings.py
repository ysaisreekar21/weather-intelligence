...
"""
# Ensure packages are installed and available in the current Python environment
...
def insert_embeddings(document_id: str, chunks_with_embeddings: List[Tuple[int, str, List[float]]]):
    """Insert embeddings into weather_embeddings table.
...
def process_document(doc: dict, model: SentenceTransformer) -> int:
    ...
    insert_embeddings(document_id, chunks_with_embeddings)
...
def ingest_embeddings(batch_limit: int = BATCH_SIZE):
    ...
    logger.info(f"Fetching up to {batch_limit} unembedded documents...")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Ingest weather document embeddings')
    parser.add_argument(
        '--batch-limit',
        type=int,
        default=BATCH_SIZE,
